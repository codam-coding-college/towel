...
- Bin wrappers: created in `~/.local/bin/<cmd>`, execute `towel exec <cmd> "$@"`, and can self-remove after failure prompt.
- Desktop wrappers: copied to `~/.local/share/applications/towel-*.desktop`, `Exec=` rewritten to `towel exec ...`, `TryExec` rewritten to towel binary, desktop db refreshed.
