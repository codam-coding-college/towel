...
- Runtime CLI entrypoint is `user/local/share/towel/bin/towel`; command implementations are split across `user/local/share/towel/lib/run` (container lifecycle + exec/remove), `user/local/share/towel/lib/install` (install flow + prompt semantics), and `user/local/share/towel/lib/export` (host wrapper generation/cleanup). Help text is in `user/local/share/towel/lib/help`.
...
- Export wrappers are now generated by towel itself (`lib/export`), not by `distrobox-export` patching. Bin wrappers call `towel exec <cmd> "$@"` and may self-remove on failure; desktop wrappers are copied to `~/.local/share/applications/towel-*.desktop` with rewritten `Exec=`.
